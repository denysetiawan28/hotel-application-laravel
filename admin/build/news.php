        ...
        <script type="text/javascript" charset="utf-8">
            $(document).ready(function(){
				...
				$(document).on('click','.remove',function() {
					...
					$('.btn-delete').attr({href: 'doDeleteNews.php?idNews=' + id});
				});
            })
...
<div class="btn-toolbar">
    <a href="addNews.php"><button class="btn btn-primary"><i class="icon-plus"></i> News</button></a>
  <div class="btn-group">
  </div>
</div>
<div>
    <table id="datatables" class="display">
      ...
      <tbody>
      	<?php
			$result = mysql_query("SELECT * FROM news WHERE Status='Active'");
			while($row = mysql_fetch_array($result)){
		?>
        <tr>
          <td><?php echo $row['ID_News']; ?></td>
          <td><?php echo $row['Title']; ?></td>
          <td><a href="<?php echo $pathNews.$row['File']; ?>", target="_blank"><img src="<?php echo $pathNews.$row['File']; ?>" width="100px" height="100px"></a></td>
          <td><?php echo $row['Description']; ?></td>
          <td><?php echo $row['Date']; ?></td>
          <td>
              <a href="updateNews.php?idNews=<?php echo $row['ID_News']; ?>"><i class="icon-pencil"></i></a>
...
<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-body">
        <p class="error-text"><i class="icon-warning-sign modal-icon"></i>Are you sure you want to delete the news?</p>
    </div>
