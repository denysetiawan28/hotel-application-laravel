        ...
        <script type="text/javascript" charset="utf-8">
            $(document).ready(function(){
				...
				$(document).on('click','.remove',function() {
					...
					$('.btn-delete').attr({href: 'doDeleteEvents.php?idEvents=' + id});
				});
            })
...
<div class="btn-toolbar">
    <a href="addEvents.php"><button class="btn btn-primary"><i class="icon-plus"></i> Events</button></a>
  <div class="btn-group">
  </div>
</div>
<div>
    <table id="datatables" class="display">
      ...
      <tbody>
      	<?php
			$result = mysql_query("SELECT * FROM events WHERE Status='Active' ");
			while($row = mysql_fetch_array($result))
			{
		?>
        <tr>
          <td><?php echo $row['ID_Events']; ?></td>
          <td><?php echo $row['Title']; ?></td>
          <td><a href="<?php echo $pathEvents.$row['File']; ?>", target="_blank"><img src="<?php echo $pathEvents.$row['File']; ?>" width="100px" height="100px"></a></td>
          <td><?php echo $row['Description']; ?></td>
          <td><?php echo $row['Date']; ?></td>
          <td><?php echo $row['Time']; ?></td>
          <td>
              <a href="updateEvents.php?idEvents=<?php echo $row['ID_Events']; ?>"><i class="icon-pencil"></i></a>
...
<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-body">
        <p class="error-text"><i class="icon-warning-sign modal-icon"></i>Are you sure you want to delete the events?</p>
    </div>
