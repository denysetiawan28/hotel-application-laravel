        ...
        <script type="text/javascript" charset="utf-8">
            $(document).ready(function(){
				...
				$(document).on('click','.remove',function() {
					...
					$('.btn-delete').attr({href: 'doDeleteAdditional.php?idAdd=' + id});
				});
            })
...
<div class="btn-toolbar">
    ...
    <a href="addAdditional.php"><button class="btn btn-primary"><i class="icon-plus"></i> New Additional</button></a>
  <div class="btn-group">
  </div>
</div>
<div>
    <table id="datatables" class="display">
      ...
      <tbody>
      	<?php
			$result = mysql_query("SELECT * FROM additional WHERE Status='Active'");
			while($row = mysql_fetch_array($result)){
		?>
        <tr>
          <td><?php echo $row['ID_Additional']; ?></td>
          <td><?php echo $row['Additional_Name']; ?></td>
          <td><?php echo $row['Price']; ?></td>
          <td><?php echo $row['Description']; ?></td>
          <td><?php echo $row['Quantity']; ?></td>
          <td>
              <a href="updateAdditional.php?idAdd=<?php echo $row['ID_Additional']; ?>"><i class="icon-pencil"></i></a>
...
<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-body">
        <p class="error-text"><i class="icon-warning-sign modal-icon"></i>Are you sure you want to delete the additional?</p>
    </div>
