        ...
        <script type="text/javascript" charset="utf-8">
            $(document).ready(function(){
				...
				$(document).on('click','.remove',function() {
					...
					$('.btn-delete').attr({href: 'doDeleteOffer.php?idOffer=' + id});
				});
            })
...
<div class="btn-toolbar">
    <a href="addOffer.php"><button class="btn btn-primary"><i class="icon-plus"></i> Offer</button></a>
  <div class="btn-group">
  </div>
</div>
<div>
    <table id="datatables" class="display">
      ...
      <tbody>
      	<?php
			$result = mysql_query("SELECT * FROM offer WHERE Status='Active'");
			while($row = mysql_fetch_array($result)){
		?>
        <tr>
          <td><?php echo $row['ID_Offer']; ?></td>
          <td><?php echo $row['Title']; ?></td>
          <td><a href="<?php echo $pathOffer.$row['File']; ?>", target="_blank"><img src="<?php echo $pathOffer.$row['File']; ?>" width="100px" height="100px"></a></td>
          <td><?php echo $row['Description']; ?></td>
          <td><?php echo $row['From_Date']; ?></td>
          <td><?php echo $row['Until_Date']; ?></td>
          <td>
              <a href="updateOffer.php?idOffer=<?php echo $row['ID_Offer']; ?>"><i class="icon-pencil"></i></a>
...
<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-body">
        <p class="error-text"><i class="icon-warning-sign modal-icon"></i>Are you sure you want to delete the offer?</p>
    </div>
